...
if ($row['count'] > 0) {
    ...
    $sql = "UPDATE umbral_ambiente SET temperatura_min = ?, temperatura_max = ?, humedad_min = ?, humedad_max = ? WHERE id_usuario = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ddddi", $temperature_min, $temperature_max, $humidity_min, $humidity_max, $id_usuario);
} else {
    // Si no existe, hacer INSERT
    $sql = "INSERT INTO umbral_ambiente (id_usuario, temperatura_min, temperatura_max, humedad_min, humedad_max)
            VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
