...
while ($umbral = $result_umbral->fetch_assoc()) {
    ...
    if ($result_datos->num_rows > 0) {
        ...
        if ($humedad < $humedad_min) {
            $mensaje_email .= "<p>⚠️ La humedad está por debajo del umbral mínimo. (Sensor suelo)</p>";
        }
        if ($humedad > $humedad_max) {
            $mensaje_email .= "<p>⚠️ La humedad está por encima del umbral máximo. (Sensor suelo)</p>";
        }
        if ($temperatura < $temperatura_min) {
            $mensaje_email .= "<p>⚠️ La temperatura está por debajo del umbral mínimo. (Sensor suelo)</p>";
        }
        if ($temperatura > $temperatura_max) {
            $mensaje_email .= "<p>⚠️ La temperatura está por encima del umbral máximo. (Sensor suelo)</p>";
        }
