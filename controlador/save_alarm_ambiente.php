...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sqlCheck = "SELECT COUNT(*) AS count FROM umbral_ambiente WHERE id_usuario = ?";
    ...
    if ($row['count'] > 0) {
        ...
        $sqlUpdate = "UPDATE umbral_ambiente 
                      SET humedad_min = ?, humedad_max = ?, temperatura_min = ?, temperatura_max = ? 
                      WHERE id_usuario = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("ddddi", $humedad_min, $humedad_max, $temperatura_min, $temperatura_max, $id_usuario);
        ...
        $sqlInsert = "INSERT INTO umbral_ambiente (id_usuario, humedad_min, humedad_max, temperatura_min, temperatura_max)
                      VALUES (?, ?, ?, ?, ?)";
        $stmtInsert = $conn->prepare($sqlInsert);
        $stmtInsert->bind_param("idddd", $id_usuario, $humedad_min, $humedad_max, $temperatura_min, $temperatura_max);
